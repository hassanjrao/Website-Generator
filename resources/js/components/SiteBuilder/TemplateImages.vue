<template>
  <v-app app>
    ...
    <v-card-title class="d-flex justify-content-between">
      ...
      <v-btn color="primary" :loading="loading" @click="submitSiteTemplateImages"
        >Submit</v-btn
      >
    </v-card-title>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-row>
                ...
                >
                  <v-file-input
                    label="Product Section BG Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="productSectionBgImage"
                ...
                >
                  <v-file-input
                    label="Popular Product Section Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="popularProductsectionImage"
                ...
                <v-col
                  ...
                  sm="6"
                  md="4"
                  v-if="includedLayoutItems.includes('relatedProductsection')"
                >
                  <v-file-input
                    label="Related Product Section Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="relatedProductsectionImage"
                ...
                >
                  <v-file-input
                    label="CTA Section BG Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="ctaSectionBgImage"
                ...
                >
                  <v-file-input
                    label="Contact Section BG Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="contactSectionBgImage"
                ...
                <v-col
                  ...
                  md="4"
                  v-if="
                    includedLayoutItems.includes('aboutSection') && aboutsectionimages > 0
                  "
                >
                  <!-- <div v-for="(image,index) in aboutSectionImageArr">

                                        <v-file-input :label="image.label"
                                        show-size truncate-length="15" required
                                            v-model="image.model" >
                                        </v-file-input>
                  ...
                  <v-file-input
                    label="About Section BG Image(s)"
                    show-size
                    truncate-length="15"
                    required
                    v-model="aboutSectionBgImages"
                    multiple
                    @input="$v.aboutSectionBgImages.$touch()"
                    :error-messages="aboutSectionBgImagesErrors"
                    @blur="$v.aboutSectionBgImages.$touch()"
                  ></v-file-input>
                </v-col>
                ...
                >
                  <v-file-input
                    label="Hero Section BG Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="heroSectionBgImage"
                    @input="$v.heroSectionBgImage.$touch()"
                    :error-messages="heroSectionBgImageErrors"
                    @blur="$v.heroSectionBgImage.$touch()"
                  ></v-file-input>
                </v-col>
                ...
                >
                  <v-file-input
                    label="Hero Section Product Image"
                    show-size
                    truncate-length="15"
                    required
                    v-model="heroSectionProductImage"
                    @input="$v.heroSectionProductImage.$touch()"
                    :error-messages="heroSectionProductImageErrors"
                    @blur="$v.heroSectionProductImage.$touch()"
                  ></v-file-input>
                </v-col>
              </v-row>
            </v-card-text>
...
<script>
import { required, requiredIf, email, withParams } from "vuelidate/lib/validators";
...
export default {
  ...
  validations: {
    ...
    aboutSectionBgImages: {
      requiredIf: requiredIf(function () {
        return this.includedLayoutItems.includes("aboutSection");
      }),
      fileSize(files) {
        if (this.aboutsectionimages > 0) {
          if (!Array.isArray(files) || files.length !== this.aboutsectionimages) {
            return false;
          }
        }
        return true;
  ...
  computed: {
    heroSectionBgImageErrors() {
      ...
      !this.$v.heroSectionBgImage.requiredIf &&
        errors.push("Hero Section Background Image is required.");
      return errors;
    },
    heroSectionProductImageErrors() {
      const errors = [];
      if (!this.$v.heroSectionProductImage.$dirty) return errors;
      !this.$v.heroSectionProductImage.requiredIf &&
        errors.push("Hero Section Product Image is required.");
      return errors;
    },
    productSectionBgImageErrors() {
      const errors = [];
      if (!this.$v.productSectionBgImage.$dirty) return errors;
      !this.$v.productSectionBgImage.requiredIf &&
        errors.push("Product Section Background Image is required.");
      return errors;
    },
    popularProductsectionImageErrors() {
      const errors = [];
      if (!this.$v.popularProductsectionImage.$dirty) return errors;
      !this.$v.popularProductsectionImage.requiredIf &&
        errors.push("Popular Product Section Image is required.");
      return errors;
    },
    relatedProductsectionImageErrors() {
      const errors = [];
      if (!this.$v.relatedProductsectionImage.$dirty) return errors;
      !this.$v.relatedProductsectionImage.requiredIf &&
        errors.push("Related Product Section Image is required.");
      return errors;
    },
    aboutSectionBgImagesErrors() {
      const errors = [];
      if (!this.$v.aboutSectionBgImages.$dirty) return errors;
      !this.$v.aboutSectionBgImages.requiredIf &&
        errors.push("About Section Background Image is required.");
      this.aboutsectionimages>0 && !this.$v.aboutSectionBgImages.fileSize &&
        errors.push(
          "About Section Background Image should be " + this.aboutsectionimages + " images."
        );
      return errors;
    },
    contactSectionBgImageErrors() {
      ...
      !this.$v.contactSectionBgImage.requiredIf &&
        errors.push("Contact Section Background Image is required.");
      return errors;
    },
    ctaSectionBgImageErrors() {
      const errors = [];
      if (!this.$v.ctaSectionBgImage.$dirty) return errors;
      !this.$v.ctaSectionBgImage.requiredIf &&
        errors.push("CTA Section Background Image is required.");
      return errors;
    },
  },
  ...
  methods: {
    submitSiteTemplateImages() {
      ...
      console.log("afet v touch");

      if (this.$v.$error) {
        return;
      }

      console.log("afet v err");
      ...
      if (this.popularProductsectionImage) {
        formData.append(
          "popular_product_section_bg_image",
          this.popularProductsectionImage
        );
      }
      if (this.relatedProductsectionImage) {
        formData.append(
          "related_product_section_bg_image",
          this.relatedProductsectionImage
        );
      }

      console.log("abotu section bg images---->", this.aboutSectionBgImages);
      if (this.aboutSectionBgImages) {
        formData.append("about_section_bg_images", this.aboutSectionBgImages);

        for (let i = 0; i < this.aboutSectionBgImages.length; i++) {
          formData.append("about_section_bg_images[]", this.aboutSectionBgImages[i]);
        }
      }
      ...
      axios
        .post("/sites/submit-site-template-images", formData, {
    ...
    getIncludedLayoutItems(includedLayoutItems) {
      ...
      if (
        !this.includedLayoutItems.includes("aboutSection") ||
        this.aboutsectionimages == 0
      ) {
        this.selectedAboutSection = null;
        this.aboutSectionBgImages = null;
      }
  ...
  created() {
    ...
    console.log("about imagesss",this.aboutsectionimages);
